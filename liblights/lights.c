...
static int write_string(const char *path, const char *value)
{
	...
	if (fd < 0) {
		if (already_warned == 0) {
			ALOGV("write_string failed to open %s\n", path);
			already_warned = 1;
		}
		return -1;
	}
...
static int read_int(const char *path)
{
	...
	if (fd < 0) {
		if (already_warned == 0) {
			ALOGV("read_int failed to open %s\n", path);
			already_warned = 1;
		}
		return -1;
	}
...
#define BACKLIGHT_CLASS "/sys/devices/i2c-0/0-0036/leds/lm3533-lcd-bl/"
static void led_desc_enumerate(void)
{
	...
	dir = opendir("/sys/devices/i2c-0/0-0036/leds");
	if (dir == NULL)
		return;
